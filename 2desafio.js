...
//------------ Uso de filter y map --------------
const mujeres = datos.filter(
  (usuario) => usuario.gender === "female" && usuario.bloodGroup === "A+"
);

const nombresYApellidosMujeres = mujeres.map(
  (mujer) => `${mujer.firstName} ${mujer.lastName}`
);
...
console.log(
  `La cantidad de personas menores de 30 años es: ${contadorMenoresDe30}`
);
